...
class RippleShapeFrameLayout @JvmOverloads constructor(
    context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0
) : ShapeFrameLayout(context, attrs, defStyleAttr) {
    ...
    private fun parseAttrs(context: Context, attrs: AttributeSet?) {
        context.withStyledAttributes(attrs, R.styleable.RippleShapeFrameLayout) {
            val rippleColor = getColor(
                R.styleable.RippleShapeFrameLayout_shape_ripple_color,
                ShapeConstants.DEFAULT_RIPPLE_COLOR
            )
            this@RippleShapeFrameLayout.rippleColor = rippleColor


            val rippleRadius =
                if (hasValue(R.styleable.RippleShapeFrameLayout_shape_ripple_radius)) {
                    val radiusAttrValue = getDimension(
                        R.styleable.RippleShapeFrameLayout_shape_ripple_radius, 0f
                    )
                    floatArrayOf(
            ...
            this@RippleShapeFrameLayout.rippleRadius = rippleRadius


            val scaleOnPress = getBoolean(
                R.styleable.RippleShapeFrameLayout_scale_on_press,
                false
            )
            this@RippleShapeFrameLayout.enableScaleOnPress = scaleOnPress
        }
    }
