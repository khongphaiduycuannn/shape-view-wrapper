...
class RippleShapeView @JvmOverloads constructor(
    context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0
) : ShapeView(context, attrs, defStyleAttr) {
    ...
    private fun parseAttrs(context: Context, attrs: AttributeSet?) {
        context.withStyledAttributes(attrs, R.styleable.RippleShapeView) {
            val rippleColor = getColor(
                R.styleable.RippleShapeView_shape_ripple_color,
                ShapeConstants.DEFAULT_RIPPLE_COLOR
            )
            this@RippleShapeView.rippleColor = rippleColor


            val rippleRadius =
                if (hasValue(R.styleable.RippleShapeView_shape_ripple_radius)) {
                    val radiusAttrValue = getDimension(
                        R.styleable.RippleShapeView_shape_ripple_radius, 0f
                    )
                    floatArrayOf(
            ...
            this@RippleShapeView.rippleRadius = rippleRadius


            val scaleOnPress = getBoolean(
                R.styleable.RippleShapeView_scale_on_press,
                false
            )
            this@RippleShapeView.enableScaleOnPress = scaleOnPress
        }
    }
