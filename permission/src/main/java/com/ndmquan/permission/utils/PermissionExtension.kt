...
private fun AppCompatActivity.createSettingIntent(permission: String?): Intent {
    return when (permission) {
        Manifest.permission.SYSTEM_ALERT_WINDOW -> Intent(Settings.ACTION_MANAGE_OVERLAY_PERMISSION).apply {
            data = Uri.fromParts("package", packageName, null)
        }

        Manifest.permission.POST_NOTIFICATIONS -> Intent().apply {
            when {
        ...
        Manifest.permission.WRITE_SETTINGS -> Intent(Settings.ACTION_MANAGE_WRITE_SETTINGS).apply {
            data = Uri.fromParts("package", packageName, null)
        }
